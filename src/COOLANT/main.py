...
def train_one_epoch(similarity_module, detection_module, clip_module,
                    optim_task_similarity, optim_task_detection, optimizer_task_clip,
                    loader, device):
    ...
    for i, (text, image, label) in enumerate(loader):
        ...
        image_loss = soft_loss(logits, func.softmax(soft_label, 1))
        caption_loss = soft_loss(logits.T, func.softmax(soft_label.T, 1))
...
@torch.no_grad()
def validate(clip_module, detection_module, loader, device):
    clip_module.eval()
...
def train(train_loader, val_loader, test_loader, device):
    ...
    best_clip_state = clip_module.state_dict()
    for key, value in best_clip_state.items():
        best_clip_state[key] = value.clone()
    no_up_limits = 10
    no_up = 0
    ...
    for epoch in pbar:
        train_loss, train_acc, train_micro, train_macro = train_one_epoch(similarity_module, detection_module,
                                                                          ...
                                                                          optimizer_task_clip,
                                                                          train_loader, device)
        val_loss, val_acc, val_micro, val_macro, _, _, _ = validate(clip_module, detection_module,
                                                                    val_loader, device)
    ...
    test_loss, test_acc, test_micro, test_macro, all_truth, all_preds, all_score = validate(clip_module,
                                                                                            detection_module,
                                                                                            test_loader, device)
...
def main():
    ...
    dataset = MyDataset(dataset_name, device)
